...
class Xlsx2csv {
    ...
    public function __construct($file = '', $throttle = ''){
        ...
        if(!is_dir($this->bin_path.'converter'))
        {mkdir($this->bin_path.'converter', 0777);};
    }
    public function convert($file = '', $throttle = 0){
        ...
        $strings = array();
        $dir = getcwd() . '/';
        $filename = "file://" .$dir.str_replace('/', '', $this->bin_path)."/converter/xl/sharedStrings.xml";
        $z = new XMLReader;
        $z->open($filename);
        $doc = new DOMDocument;
        ...
        $z->close($filename);
        $dir = getcwd() . '/';
        $filename = "file://" .$dir.str_replace('/', '', $this->bin_path)."/converter/xl/worksheets/sheet1.xml";
        $z = new XMLReader;
        $z->open($filename);
        $doc = new DOMDocument;
        $rowCount="0";
    ...
    function my_fputcsv($handle, $fields, $delimiter = ';', $enclosure = '"', $escape = '\\'){
        ...
        foreach ($fields as $field) {
            ...
            if (strpbrk($f, " \t\n\r".$delimiter.$enclosure.$escape) || strchr($f, "\000")){
                fwrite($handle, mb_convert_encoding($enclosure.$f.$enclosure, 'windows-1251', 'utf-8') );
            }
            else {
                fwrite($handle,  mb_convert_encoding($f, 'windows-1251', 'utf-8'));
            }
            $first = 0;
        }
        fwrite($handle, "\n");
